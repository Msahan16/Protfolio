...
/*
  This file contains two components in one file for easy preview:
  - LiquidEther (the WebGL liquid background)
  - Hero (your original hero that uses LiquidEther as background)

  If you'd rather keep components in separate files, move the LiquidEther
  component to `LiquidEther.jsx` and import it into `Hero.jsx`.
*/
...
export function LiquidEther({
  ...
  useEffect(() => {
    ...
    const face_vert = `attribute vec3 position;\nuniform vec2 px;\nuniform vec2 boundarySpace;\nvarying vec2 uv;\nprecision highp float;\nvoid main(){\nvec3 pos = position;\nvec2 scale = 1.0 - boundarySpace * 2.0;\npos.xy = pos.xy * scale;\nuv = vec2(0.5)+(pos.xy)*0.5;\ngl_Position = vec4(pos, 1.0);\n}`;
    const line_vert = `attribute vec3 position;\nuniform vec2 px;\nprecision highp float;\nvarying vec2 uv;\nvoid main(){\nvec3 pos = position;\nuv = 0.5 + pos.xy * 0.5;\nvec2 n = sign(pos.xy);\npos.xy = abs(pos.xy) - px * 1.0;\npos.xy *= n;\ngl_Position = vec4(pos, 1.0);\n}`;
    const mouse_vert = `precision highp float;\nattribute vec3 position;\nattribute vec2 uv;\nuniform vec2 center;\nuniform vec2 scale;\nuniform vec2 px;\nvarying vec2 vUv;\nvoid main(){\nvec2 pos = position.xy * scale * 2.0 * px + center;\nvUv = uv;\ngl_Position = vec4(pos, 0.0, 1.0);\n}`;
...
export default function Hero() {
  return (
    <section className="hero galaxy-bg">
      {/* Liquid background placed behind content */}
      <LiquidEther className="hero-liquid" colors={["#2E1BFF", "#FF6FD8", "#A08BFF"]} autoDemo autoSpeed={0.6} style={{ position: 'absolute', inset: 0, zIndex: 0 }} />
...
/* ===== Minimal CSS you should add to Hero.css (or import separately) =====
.liquid-ether-container { position: absolute; inset: 0; width: 100%; height: 100%; pointer-events: none; }
.hero { position: relative; min-height: 520px; display: flex; align-items: center; }
.galaxy-bg { background: radial-gradient(ellipse at center, rgba(255,255,255,0.02), rgba(0,0,0,0.2)); }
.hero-container { width: 100%; max-width: 1200px; margin: 0 auto; padding: 60px 24px; }
.hero-content { display: flex; gap: 30px; align-items: center; justify-content: space-between; }
.text-content { flex: 1; color: #eaf2ff; }
.hero-title { font-size: 2.4rem; margin: 0 0 8px; }
.highlight { color: var(--primary-blue, #66a0ff); }
.hero-subtitle { font-size: 1.25rem; margin: 0 0 16px; opacity: 0.9; }
.hero-description { max-width: 54ch; line-height: 1.6; opacity: 0.95; }
.hero-buttons { margin-top: 22px; display: flex; gap: 12px; }
.btn { position: relative; padding: 10px 18px; border-radius: 10px; text-decoration: none; }
.btn-primary { background: linear-gradient(90deg,#0066ff,#00ccff); color: #fff; }
.btn-secondary { background: rgba(255,255,255,0.06); color: #fff; border: 1px solid rgba(255,255,255,0.06); }
.hero-image { width: 360px; flex: 0 0 360px; }
.image-wrapper { position: relative; }
.hero-img { width: 100%; display: block; }
.image-glow { position: absolute; inset: -10px; filter: blur(40px); opacity: 0.65; pointer-events: none; }

Add the CSS above into your existing Hero.css. Adjust measurements and colors to taste.
*/
